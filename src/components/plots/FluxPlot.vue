...
const peak = computed(() => Math.max(...props.triage.frames.map((f) => f.targetFlux)))
const underFloor = computed(() => peak.value < props.triage.floor)
const yMax = computed(() => (underFloor.value ? 260 : 1200))
const gridlines = computed(() => (underFloor.value ? [0, 100, 200] : [0, 400, 800, 1200]))
const sc = computed(() => axes(frame, [0, 1], [0, yMax.value]))
