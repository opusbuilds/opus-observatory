...
function target(
  ...
  a: Ephemeris,
  e: Ephemeris,
  counts: [opened: number, fitted: number, floorNights: number],
  note: string | null,
  faint = false,
): Target {
  const [opened, fitted, floorNights] = counts
  return { name, slug: slugify(name), vmag, periodDays, archive: a, exoclock: e, derived, note, faint, opened, fitted, floorNights }
}
